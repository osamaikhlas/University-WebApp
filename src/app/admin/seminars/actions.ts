...
const PERMISSIONS = MODULE_PERMISSIONS.seminars;
const ENTITY_TYPE = "Seminar";

const seminarSchema = z.object({
  title: z.string().trim().min(1, "Title is required").max(300),
  departmentId: z.string().trim().optional(),
  description: z.string().trim().max(4000).optional(),
  speaker: z.string().trim().max(300).optional(),
  startDate: requiredDateField,
  endDate: optionalDateField,
  venue: z.string().trim().max(300).optional(),
});

export type SeminarFormState = { error: string | null };

function parseForm(formData: FormData) {
  return seminarSchema.safeParse({
...
export async function createSeminar(
  ...
  redirect(`/admin/seminars/${seminar.id}`);
}
...
export async function updateSeminar(
  ...
  const before = await prisma.seminar.findUnique({ where: { id } });
  if (!before) {
  ...
  const after = await prisma.seminar.update({
    where: { id },
    data: {
      title: parsed.data.title,
      departmentId: department.departmentId,
...
export async function transitionSeminar(
  id: string,
  ...
  const seminar = await prisma.seminar.findUniqueOrThrow({ where: { id } });
  ...
  try {
    await applyWorkflowTransition({
      ...
      actorId: user.id,
      comment,
      update: (data) => prisma.seminar.update({ where: { id }, data }),
    });
  } catch (error) {
    if (!(error instanceof WorkflowError)) throw error;
    redirect(`/admin/seminars/${id}?workflowError=${encodeURIComponent(error.message)}`);
  }
